...
fun ImageView.setImgFile(file: File) {
    //.placeholder(R.drawable.place_pro).error(R.drawable.place_pro)
    Glide.with(this).load(file).apply(RequestOptions().transform(CenterCrop(), CircleCrop()))
        .into(this)
}
...
fun buildURL(url: String, params: Array<out Pair<String, Any?>>): String {
...
fun request(build: Request.Builder): String? {
    return LHttp.client.newCall(build.build()).execute().body?.string()
}

fun readJson(build: Request.Builder): JSONObject? {
    return try {
        JSONObject(LHttp.client.newCall(build.build()).execute().body?.string())
    } catch (e: IOException) {
...
fun Context.getString(key: String) =
    this.getString(this.resources.getIdentifier(key, "string", this.packageName)) ?: ""
